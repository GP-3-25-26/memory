...
\begin{itemize}
    ...
    \begin{itemize}
        \item el usuario expresa que el sistema ayuda a tomar decisiones clínicas en cuanto a diagnosticar si sus pacientes tienen predisposición a ciertas en enfermedades.
    \end{itemize}
    ...
    \begin{itemize}
        \item El usuario expresa que el impacto del código de colores es positivo para apreciar de manera rápida la diferencia entre variantes detectadas
    \end{itemize}

    \item \textbf{¿Qué error sería “inaceptable” que la herramienta cometiera?}
    \begin{itemize}
        \item el usuario expresa que sería inaceptable que se equivocase en el porcentaje de probabilidad a la hora de dar una predicción.
    \end{itemize}

    \item \textbf{Preferirías: Menos predicciones pero más seguras, o más predicciones aunque alguna sea dudosa.}
    \begin{itemize}
        \item El usuario expresa su predilección por una mayor cantidad de predicciones dudosas indicando el porcentaje de probabilidad sea menor ya que eso hace que el abanico de diagnóstico sea mucho más amplio, evitando obviar escenarios menos probables.
    \end{itemize}
    ...
    \begin{itemize}
        \item El usuario percibió al asistente como una herramienta de consulta rápida.
    \end{itemize}
    ...
    \begin{itemize}
        \item El usuario expresa que pediría al asistente información sobre artículos relacionados a las enfermedades relacionadas con las predicciones.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textit{Pedir explicación de la predicción.} (seleccionado por el usuario)
        \item \textit{Pedir siguientes pasos / pruebas recomendadas.} (seleccionado por el usuario)
        \item No lo usaría.
    \end{itemize}

    \item \textbf{¿Recomendarías esta herramienta a un profesional? ¿Por qué?}
    \begin{itemize}
        \item El usuario sí recomendaría la herramienta a otros profesionales porque considera que ahorra mucho el tiempo y permite dar un diagnóstico a más pacientes de manera veloz.
    \end{itemize}

    \item \textbf{Si mañana desapareciera, ¿qué echarías de menos (si algo)?}
    \begin{itemize}
        \item El usuario expresa que echaría de menos la rapidez con la que se ejecuta un posible diagnóstico, expresando la importancia de valorar el tiempo posterior dedicado a explorar dicho diagnóstico.
    \end{itemize}

    \item \textbf{¿Qué debería pasar para que esto sea “imprescindible” en vuestro trabajo?}
    \begin{itemize}
        \item El usuario considera que para que eso ocurra, debería desaparecer el acceso a todas las demás fuentes de información, por ejemplo artículos científicos relacionados a la predicción de enfermedades genéticas, lo que obligaría a utilizar la aplicación por su velocidad.
    \end{itemize}
\end{itemize}
...
\underline{\textbf{Puntos fuertes (validado)}}
\begin{itemize}
...
\underline{\textbf{Identificación de brechas y áreas de mejora}}
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Solución propuesta:} Implementar una barra lateral o cabecera con los datos clave del paciente (Fenotipo, Antecedentes) accesible en las pantallas de Variantes y Predicción.
    \end{itemize}

    \item \textbf{Fiabilidad y confianza}
    \begin{itemize}
        \item \textbf{Problema:} La caja negra (modelo) genera desconfianza si faltan datos. Para confiar en la predicción, el usuario exige ver por qué (geografía, antecedentes familiares).
    ...
    \begin{itemize}
        ...
        \item \textbf{Acción:} Añadir funcionalidad de notas rápidas en cada variante o etapa del análisis.
    \end{itemize}
\end{itemize}

\newpage
\underline{\textbf{Perfil psicológico del usuario (insights)}}
\begin{itemize}
...
\underline{\textbf{Conclusión de la validación}}
\begin{itemize}
...
\underline{\textbf{Recomendaciones de iteración (next steps)}}
\begin{itemize}
    ...
    \item \textbf{Medium priority:} Integrar un botón \textit{Buscar Artículos Relacionados} en el Chat o en la tarjeta de predicción.
    \item \textbf{Medium priority:} Implementar Auto-guardado de comentarios.
\end{itemize}

\underline{\textbf{Resumen final}}
\begin{itemize}
    \item Existe una clara demanda de mayor contexto clínico (historial, antecedentes) visible durante todo el proceso, no solo al inicio. 
    \item La confianza en la herramienta se basa en la transparencia (explicabilidad, porcentajes) y la precaución (preferencia por falsos positivos antes que falsos negativos).
\end{itemize}
...
\begin{quote}
«Analiza las fases 1 y 2 del documento GP\_DesignThinking.pdf y cuando sepas cual es el problema imagina que eres un genetista. Una vez hecho eso, ve a Chrome, abre el navegador en http://localhost:5173/ y prueba el producto.
...
¿Para qué usarías el chat dentro de la app? (elige una o varias)
...
\begin{figure}[H]
    ...
    \includegraphics[width=1\textwidth]{figures/testear/juego_roles_paso2.jpg}
...
Finalmente, se volvió a hacer uso de IA generativa para elaborar una infografía que resume visualmente todo lo trabajado durante la fase de Testear, condensando las ideas, decisiones y resultados obtenidos en esta etapa.

\begin{figure}[H]
    \centering
    \captionsetup{justification=centering}
    \includegraphics[width=1\textwidth]{figures/testear/infografia.jpg}
